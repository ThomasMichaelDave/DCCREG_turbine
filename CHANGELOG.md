...
### Added
- **Cross-section render upgrade (xsec v0.2).** Extends `drawCrossSection` only — no solver/producer edits.
  ...
  - **Reluctance motor in the meridional cut (new step 5e).** Draws the 2-of-6 rotor steel poles (45° bicone → diamond profile, `--cem-core`) and 2-of-12 stator C-EMs (U-core `--cem-core` + copper flanks `--cem-coil`) the section plane passes through, across the **axial** motor gap sourced from Block-D `demClearanceMm`. Honest 2-each idealisation; pole on rotor, C-EM on stator, coupled only by the gap. Optional N/S chips behind `dlabels`.
