...
REMOVE={f"C_AR{i}" for i in range(1,7)}|{"C__AR4"}|{f"C_BR{i}" for i in range(1,7)}  # the brigade DC-block/transfer CAPS
ISLAND={"Lx3","Lx4","Cx3","Cx4"}
COIL={f"L_A{i}" for i in range(1,7)}|{f"L_B{i}" for i in range(1,7)}   # stator electromagnet coils (Block D C-EMs) -- ESSENTIAL
GAPS={"SG1","SG2","SG3a1","SG3b1","SG4a1","SG4b1","BS3","BS4"}
def kind(ref,lib):
    if ref in REMOVE: return 'remove'
    if ref in COIL: return 'coil'
    ...
    return 'other'
COL={'remove':'#e5484d','coil':'#e08a2a','island':'#2a9d8f','gap':'#b8975a','varicap':'#7cd0ff','cap':'#5a7a96','ind':'#8a9aa8','other':'#888'}
...
leg=[Patch(fc='#fde2e2',ec='#e5484d',lw=2,label='REMOVE — brigade caps C_AR1-6 / C_BR1-6 (×12, the DC-block/transfer caps)'),
     Patch(fc=COL['coil'],label='KEEP — stator electromagnet COILS L_A1-6 / L_B1-6 (Block-D C-EMs, ESSENTIAL)'),
     ...
     Patch(fc=COL['cap'],label='Ca/Cb coupling, C_R tank'),
     Patch(fc=COL['gap'],label='commutation gaps (SG*/BS*)')]
ax.legend(handles=leg,loc='upper left',fontsize=9,framealpha=0.95)
ax.set_title("DCCREG schematic — brigade simplification (corrected): remove the brigade CAPS C_AR/C_BR (red ✕);\n"
             "KEEP the stator electromagnet COILS L_A/L_B (orange, Block-D C-EMs) and the island Cx/Lx (teal). 43 → 31 components.",
             fontsize=12)
