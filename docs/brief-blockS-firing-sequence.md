...
- Four step traces: SG1, SG3, SG2, SG4 (1 = conducting). SG1+SG3 in stroke 1 (SG1 leads, SG3 at +lag), SG2+SG4 in stroke 2 (SG2 leads, SG4 at +lag). Toggle SG1/SG2 via `sfollow`.
...
- Four concentric rings (outer→inner: SG1, SG3, SG2, SG4). Within-group angular offsets: SG1 `0`, SG3 `lag`, SG2 `pitch/2`, SG4 `pitch/2 + lag`. Draw a wedge per group per ring (width = conduction-window angle). `lag` = the return→cross-couple quench gap, ≥ the return's deionisation time **[IR]/[RH]**; the ideal solver does not order the pair (membership only).
